...
def generate_furniture_image(api_key, input_image_pil, prompt_text):
    ...
    full_prompt = (
        ...
        f"【重要】提供された元の画像の画角（カメラアングル）、部屋の構造（壁、窓、床の形など）は一切変更せず、そのままの空間に新しい家具だけを合成してください。\n"
...
def main():
    ...
    if 'gemini_api_key' not in st.session_state:
        # Streamlit Secrets、もしくはローカルの環境変数から読み込む
        env_key = os.environ.get("GOOGLE_CLOUD_API_KEY", "")

        # Streamlit Secrets が設定されている場合はそちらを優先
        if "GOOGLE_CLOUD_API_KEY" in st.secrets:
            env_key = st.secrets["GOOGLE_CLOUD_API_KEY"]
